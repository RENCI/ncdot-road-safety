...
parser.add_argument('--draw_camera_plot_only', action="store_true")

args = parser.parse_args()
input_3d_proj = args.input_3d_proj
draw_camera_plot_only = args.draw_camera_plot_only

input_3d_proj_df = pd.read_csv(input_3d_proj, usecols=['PROJ_X', 'PROJ_Y', 'WORLD_X', 'WORLD_Y', 'WORLD_Z'],
                               dtype=float)

fig = plt.figure(figsize=(10, 8))
if draw_camera_plot_only:
...
if not draw_camera_plot_only:
    ...
    fig.suptitle('LIDAR road vertices in 3D world coordinate system')
plt.show()

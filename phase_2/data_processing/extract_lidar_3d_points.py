...
def extract_lidar_3d_points_for_camera(df, cam_loc, next_cam_loc, dist_th=(20, 190),
    ...
    if proj_cam_x is not None and proj_cam_y is not None:
        ...
        df['CAM_DIST'] = np.sqrt((df['X'] - proj_cam_x) ** 2 + (df['Y'] - proj_cam_y) ** 2)
    else:
    ...
    df = df[(df['bearing_diff'] < math.pi * fov / 180) & (df['CAM_DIST'] > dist_th[0]) & (df['CAM_DIST'] < dist_th[1])]
    if include_all_cols:

...
if subplot:
    ...
    ax.scatter(df['X'], pred_series, s=20, marker='o', facecolors='none', edgecolors=df['MANUAL_YN'],
               label=["guardrail_no: ", "guardrail_yes: "] + df['MANUAL_YN'].unique())
    ax.scatter(df['X'], pred_series, s=10, marker='o', facecolors=df['WRONG'], edgecolors='none',
               label=['predict_correct:', 'predict_wrong: '] + df['WRONG'].unique() + [' fill', ' fill'])
    ax.set_xlim(100, 250)
else:
    plt.scatter(df['X'], pred_series, s=20, marker='o', facecolors='none', edgecolors=df['MANUAL_YN'],
                label=["guardrail_no: ", "guardrail_yes: "] + df['MANUAL_YN'].unique())
    plt.scatter(df['X'], pred_series, s=10, marker='o', facecolors=df['WRONG'], edgecolors='none',
                label=['predict_correct:', 'predict_wrong: '] + df['WRONG'].unique() + [' fill', ' fill'])

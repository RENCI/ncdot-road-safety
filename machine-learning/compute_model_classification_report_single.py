...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_label_file', type=str,
                        #default='../server/metadata/pole/model_predict_test_unbalanced.csv',
                        default='../server/metadata/pole/round3/model_predict_test_i_as_a.csv',
                        help='Model prediction on the single images')
    parser.add_argument('--single_metrics', type=bool, default=True,
    ...
    if single_metrics:
        ...
        concat_df['ROUND_PREDICT_SINGLE'] = concat_df.apply(lambda row: 1 if row.ROUND_PREDICT >= 0.5 else 0, axis=1)
